...
#include "zsvanalysis/MuScleFitCorrector_v4_3/Functions.h"
...
class MuScleFitCorrector
{
 public:
  ...
  double getCorrectPt(const TLorentzVector &lorentzVector, const int & chg){
    ...
    pt = ( scaleFunction_->scale( pt, lorentzVector.Eta(), lorentzVector.Phi(), chg, &scaleParVec_[0]) );
    return pt;
  };
  ...
  double getSigmaPtDiffSquared(const double & pt, const double & eta){  
    double sigmaPtData = resolutionFunction_->sigmaPt(pt, eta, &resolDataParVec_[0]);
    double sigmaPtMC = resolutionFunction_->sigmaPt(pt, eta, &resolMCParVec_[0]);
 ...
 protected:
  ...
  //std::unique_ptr<double[]> scaleParArray_;
  //std::unique_ptr<double[]> resolDataParArray_;
  //std::unique_ptr<double[]> resolMCParArray_;
  ...
  // Functions
  std::unique_ptr<scaleFunctionBase<double * >> scaleFunction_;
  std::unique_ptr<resolutionFunctionBase<double * >> resolutionFunction_;
  
  // Pointer for TRandom3 access
  std::unique_ptr<TRandom3> gRandom_;
