...
Matrix::Matrix(size_t r, size_t c) : rows(r), cols(c), elements(rows*cols, 0) {}
...
Matrix get_cholesky(const Matrix & cov){
    ...
    for (size_t i = 0; i < npar; i++) {
       if((par_has_zero_cov[i] = fabs(cov(i,i) / m) < 1e-10))
            npar_reduced--;
    }
    //calculate cholesky decomposition    cov = L L^T    of the reduced covariance matrix; allow zero-size matrix:
